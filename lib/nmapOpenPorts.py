...
class NmapOpenPorts:
    ...
    def Scan(self):
        ...
        if len(nfsPorts) != 0:
            ...
            nfs_cmd = f"nmap -v -sV -p {string_nfs_ports} --script=nfs-ls.nse,nfs-statfs.nse,nfs-showmount.nse -oA {reportDir}/nmap/nfs {self.target}"
        ...
        if len(cupsPorts) != 0:
            ...
            cups_cmd = f"nmap -v -sV -Pn --script=cups-info.nse,cups-queue-info.nse -p {string_cups_ports} -oA {reportDir}/nmap/cups-enum {self.target}"
